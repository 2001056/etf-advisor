...
맥북 한 대에서 이 앱을 상주로 굴리기 위한 문서다. 프로젝트 소개와 설계 배경은 [README.md](../README.md),
전체 설계는 [기획서.md](../기획서.md)에 있다.

**접속**: `http://<맥북-LAN-IP>:3111` (같은 와이파이의 폰·PC에서도)
...
프롬프트를 고칠 때 지켜야 하는 필드명·형식 계약은 [프롬프트-시스템-계약.md](프롬프트-시스템-계약.md)에 정리돼 있다.
마크다운 파일에서 한꺼번에 넣으려면:

```bash
pnpm --dir web run prompts:load <프롬프트-md-파일>
```
...
대시보드에 카테고리별 현재 비중과 목표 비중(월 충전액에서 유도, 기본 설정 예시는 50/30/20)의 차이를 표시한다.
...
pnpm run check:avgprice   # 평단가 계산 검증 (DB 불필요)
...
pnpm run check:transfer   # 고배당 이전 원장 이동 검증 (별도 DB)
...
`web/.env.local.example`을 복사해 채운다.
...
- [x] 프롬프트 v1 적재 및 시스템 계약 정렬 — [프롬프트-시스템-계약.md](프롬프트-시스템-계약.md)
